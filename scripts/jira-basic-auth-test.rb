...
# # ------------------------------
# project = client.Project.find('PE')
# pp project
...
# # ------------------------
# ctgEpicQuery = 'project = PE AND Team = 22 AND type in (defect, story, task) AND "Epic Link" = PE-39554'
# client.Issue.jql(ctgEpicQuery, {fields: %w(summary status)}).each do |issue|
#   puts "#{issue.id} - #{issue.fields['summary']}"
# end
#
# # Print Specifc Task
# # ------------------------
ctgEpicQuery = 'project = "PE" AND fixVersion = "Mobile - Android - 2021.08.4"'
client.Issue.jql(ctgEpicQuery, {fields: %w(summary status)}).each do |issue|
  pp issue
end
#
